#!/usr/bin/env node
/**
 * NeuroRefinement Fix v3 — Watermark Rings + Research Page
 * Run from repo root: node apply-fixes-v3.js
 */
...
function replace(content, search, rep, label) {
  if (!content.includes(search)) {
    console.error(`  ❌ NOT FOUND: ${label}`);
    console.error(`     First 80 chars: "${search.substring(0, 80)}"`);
    return content;
  }
  const result = content.replace(search, rep);
  console.log(`  ✅ Replaced: ${label}`);
  return result;
}

// Reusable SVG ring block — dark stroke version (for light backgrounds)
const ringsDark = (classes) => `<svg className="${classes}" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
// Light stroke version (for dark backgrounds)
const ringsLight = (classes) => `<svg className="${classes}" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
console.log('🔧 NeuroRefinement Fix v3\n');

// ============================================================
// 1. HERO — Replace blurry circles with visible concentric rings
// ============================================================
console.log('1. Hero — replace blur circles with concentric ring watermarks');
...
page = replace(page,
  ...
  `{/* Decorative concentric ring watermarks */}
          ${ringsDark('absolute -top-20 -right-20 w-[500px] h-[500px] opacity-[0.07]')}
          ${ringsDark('absolute bottom-0 -left-16 w-[300px] h-[300px] opacity-[0.05]')}`,
  'Hero watermark rings'
);

// ============================================================
// 2. COACHING DARK SECTION — Add watermark rings
// ============================================================
console.log('2. Coaching section — add watermark rings');
...
page = replace(page,
  ...
  `<Section variant="dark" className="relative grain">
        ${ringsLight('absolute top-8 right-8 w-[350px] h-[350px] opacity-[0.06]')}
        <div className="relative z-10">
          <SectionHeader
            variant="dark"
            eyebrow="Strengths-Based Coaching"`,
  'Coaching section watermark'
);

// ============================================================
// 3. MEMBERSHIP DARK SECTION — Add watermark rings
// ============================================================
console.log('3. Membership section — add watermark rings');
  ...
  `<Section variant="dark" className="relative grain">
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          <span className="inline-block text-sm font-medium text-brand-300 uppercase tracking-wider mb-3">
            Membership`,
  `<Section variant="dark" className="relative grain">
        ${ringsLight('absolute -bottom-10 -left-10 w-[300px] h-[300px] opacity-[0.05]')}
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          <span className="inline-block text-sm font-medium text-brand-300 uppercase tracking-wider mb-3">
            Membership`,
  'Membership section watermark'
);

writeFile('src/app/page.tsx', page);

// ============================================================
// 4. RESEARCH PAGE — Full comprehensive page
// ============================================================
console.log('4. Create Research page');

writeFile('src/app/research/page.tsx', `import { Metadata } from 'next'
...
const coreResearch = [
  ...
  {
    authors: 'Verrel J, Almagor E, Schumann F, Lindenberger U, K\\u00fchn S',
    year: 2015,
        ...
        <div className="container-wide relative z-10 py-20 md:py-28">
          <div className="max-w-3xl">
            ...
            <p className="text-xl text-brand-200 leading-relaxed mb-4">
              ...
              Mosh\\u00e9 Feldenkrais. This page collects the science that informs everything we do.
            </p>
...
console.log('   git commit -m "Visible watermark rings, full Research page with references"');
